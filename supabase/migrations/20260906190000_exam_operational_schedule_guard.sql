-- SEGEMPAT · alinha a disponibilidade de provas do preview legado com a API MySQL.
-- A data operacional oficial é America/Maceio: prova futura não aparece, não abre e não pode ser submetida.
...
BEGIN
  ...
  IF v_exam.id IS NULL THEN
    RAISE EXCEPTION 'Prova indisponível, não autorizada para seu setor ou ainda não liberada';
  END IF;
  ...
  LEFT JOIN public.employees emp
    ON lower(btrim(emp.matricula)) = lower(btrim(p.matricula))
  WHERE p.id = auth.uid();

  INSERT INTO public.exam_attempts (
    exam_id, user_id, matricula, score, passed, answers
  ) VALUES (
    v_exam.id, auth.uid(), v_matricula, v_score, v_passed, p_answers
  )
