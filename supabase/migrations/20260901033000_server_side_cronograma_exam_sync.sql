-- Sincroniza o Cronograma dentro da correção oficial da prova.
-- O Operador continua sem UPDATE direto em cronograma_entries.
...
BEGIN
  ...
  LEFT JOIN public.employees emp ON emp.matricula = p.matricula
  WHERE p.id = auth.uid();
  ...
  IF v_passed AND v_matricula IS NOT NULL THEN
    ...
    WHERE ce.exam_id = v_exam.id
      AND (ce.employee_matricula = v_matricula OR (v_employee_id IS NOT NULL AND ce.employee_id = v_employee_id))
