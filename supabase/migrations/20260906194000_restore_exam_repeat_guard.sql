-- SEGEMPAT · garante que o reforço de agenda posterior não remova a proteção contra repetição após aprovação.
-- Compatibilidade exclusiva do preview legado; a API MySQL possui a regra equivalente no servidor.
...
BEGIN
  ...
  IF p_answers IS NULL OR jsonb_typeof(p_answers) <> 'object' THEN RAISE EXCEPTION 'Formato de respostas inválido'; END IF;
  ...
  IF v_exam.id IS NULL THEN RAISE EXCEPTION 'Prova indisponível, não autorizada ou ainda não liberada'; END IF;
  ...
  IF v_exam.questions IS NULL OR jsonb_typeof(v_exam.questions) <> 'array' OR jsonb_array_length(v_exam.questions) = 0 THEN
  ...
  LOOP
    ...
    IF v_type = 'Múltipla escolha' THEN
      v_correct := v_answer IS NOT NULL AND v_answer = coalesce(v_question ->> 'correct_index', '');
    ELSE
  ...
  IF v_total <= 0 THEN RAISE EXCEPTION 'Pontuação total inválida'; END IF;
  ...
  SELECT p.matricula, emp.id INTO v_matricula, v_employee_id
  FROM public.profiles p
  LEFT JOIN public.employees emp ON lower(btrim(emp.matricula)) = lower(btrim(p.matricula))
  WHERE p.id = auth.uid();

  INSERT INTO public.exam_attempts(exam_id,user_id,matricula,score,passed,answers)
  VALUES(v_exam.id,auth.uid(),v_matricula,v_score,v_passed,p_answers)
  ...
  IF v_passed AND v_matricula IS NOT NULL THEN
    ...
    UPDATE public.cronograma_entries ce
       SET status='Realizado', type='Realizado', completion_date=v_completion_date, justification=NULL
     WHERE ce.exam_id=v_exam.id
       AND (lower(btrim(ce.employee_matricula))=lower(btrim(v_matricula)) OR (v_employee_id IS NOT NULL AND ce.employee_id=v_employee_id))
